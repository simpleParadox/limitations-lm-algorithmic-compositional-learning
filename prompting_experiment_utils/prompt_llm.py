...
def generate_text(
    prompt: str, credentials: Dict, with_code=False, model="gpt-4-0613"
) -> str:
    if model == "gemini-pro":
        assert with_code == False, "Gemini does not support code interpreter"
        return generate_text(
            prompt,
            credentials["gemini_project_id"],
            credentials["gemini_project_location"],
        )
...
def generate_text_openai(prompt: str, model: str, api_key: str, with_code=False) -> str:
    ...
    if with_code == False:
        ...
        assert (
            model != "o1-preview"
        ), "OpenAI API for O1 Preview does not support code interpreter"
