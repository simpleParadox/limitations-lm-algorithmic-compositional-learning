...
def build_prompt(sample_list: List[Tuple[str,str]], config: Dict):
    for i in range(len(sample_list)):
        ...
        perm_y = perm_y.split(' ')
        assert all([f'{word}.{n_left*count}'==y for word,n_left,count,y in zip(match_seq_rev,n_lefts_rev,counts_rev,perm_y)])
        sample_list[i] = (perm_x, perm_y, perm_x_words, perm_x_start, match_seq_rev, n_lefts_rev, counts_rev)
...
Finally, output this sequence of words, in reverse order in the format word.x where x is the number of left matchings until the output word times the number of matchings until the output word. Example answer: abcd.4 efab.1 ghef.0\n\n"""
    ...
    for perm_x, perm_y, perm_x_words, perm_x_start, match_seq_rev, n_lefts_rev, counts_rev in sample_list[0:n_demonstrations]:
        new_example = f'\n\nExample: {perm_x}\n'
        if fewshot_cot==True or fewshot_cot=='enumerate':
            ...
            if fewshot_cot=='enumerate':
                ...
                new_example += '\n'
            new_example += f'Starting with "{perm_x_start}", let\'s match and calculate:\n\n'
            matching_seq = match_seq_rev[::-1] # the true answer has the matching sequence reversed, so we reverse again, to get it forward
            ...
            for i in range(1, len(matching_seq)):
                ...
                if is_left_match:
                    ...
                    if fewshot_cot=='enumerate':
                        ...
                        new_example += f'No left match, {n_left_next} left matches so far.\n'
            ...
            if fewshot_cot=='calculate' or fewshot_cot=='enumerate':
                ...
                for i in range(len(match_seq_rev)):
                    new_example += f'{match_seq_rev[i]}.{n_lefts_rev[i]*(len(n_lefts_rev)-i-1)}\n'
            new_example += f'\nThus, the answer is: "' + ' '.join(perm_y) + '".'
        else:
            new_example += f'Answer: {' '.join(perm_y)}\n\n'
        predicted_answer = ' '.join([match_seq_rev[i] + '.' + str(n_lefts_rev[i]*(len(match_seq_rev)-i-1)) for i in range(len(match_seq_rev))])
    ...
    if not config['sge']:
        ...
        insertion = f"""{description}""" + "Sequence: " + actual_perm_x + f"""{cot_question}"""
