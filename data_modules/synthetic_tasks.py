...
class PointerExecutionNeighbour:
    def __init__(self, min_len, max_len, min_hops, max_hops, sub_task="next"):
        ...
        assert self.length_low >= 2, "Configuration Error: Minimum length must be at least 2"
        ...
        self.data_choices = list(number_chars[:8]) # the choices for the middle character of "words"
    ...
    def generate_double_pointer_execution(self, n_samples):
        ...
        while len(samples) < n_samples:
            ...
            for i,pos in enumerate(positions_next):
                two_big_letters = np.random.choice(self.data_choices, size=2, replace=False)
            ...
            remaining_next_positions = np.random.permutation(
                [i for i in range(1, length) if i not in positions_next and i not in positions_confuse]
            )
            ...
            question_str = f"pe {self.task}: " + " ".join(["".join(x) for x in question_words]) + " answer: "
...
class PointerExecutionReverseMulticount:
    ...
    def generate_samples(self,n_samples):
        ...
        for _ in range(n_samples):
            ...
            question = f"prand {self.task}: " + " ".join(words) + " | " + start + " answer: "
...
class HighestSubseqenceNoNeighboursTask:
    ...
    def highest_subseq_no_neighbours(self, numbers):
        ...
        if numbers[1] > numbers[0]:
            highest_until_i[1] = (max(numbers[1],0), -1, 'y' if numbers[1] > 0 else 'n')
        else:
        ...
        for i in range(2, len(numbers)):
            if highest_until_i[i-1][0] > highest_until_i[i-2][0] + numbers[i]:
                ...
                highest_until_i[i] = (highest_until_i[i-2][0] + numbers[i], i-2, 'y' if numbers[i] > 0 else 'n')
    ...
    def generate_highest_subseq_no_neighbours(self, n_samples):
        ...
        for _ in range(n_samples):
            ...
            highest_subseq, answer_number, exec_trace = self.highest_subseq_no_neighbours(numbers)
            if self.task == "sum":
    ...
    def generate(self, n_samples):
        ...
        samples = [
            (f"hsnn {self.task}: " + " ".join([str(x) for x in numbers]) + " answer: ", answer) 
...
class MultiplicationTask:
    ...
    def generate(self, n_samples):
        ...
        samples = [
            (f"calc {self.task}: " + str(left) + calc_sign + str(right) + " answer: ", str(answer)) 
...
class CopyTask:
    def __init__(self, min_len=10, max_len=40, max_group_length=4, reverse=False):
        ...
        self.length_high = max_len + 1 # max length of the sequence, +1 to include the max_len
        self.max_group_length = max_group_length # max length of the each "word"
        self.reverse = reverse # if true, reverse the words, but not the letters in the words
    ...
    def generate_copy(self,n_samples):
        ...
        copy_samples = [("copy: " + x + " answer: ", y) for x, y in zip(samples, samples)]
        return copy_samples
    ...
    def generate_reverse(self, n_samples):
        ...
        reverse_samples = [("reverse: " + x + " answer: ", y) for x, y in zip(samples, reversed_samples)]
        return reverse_samples
    
    def generate(self, n_samples):
        return self.generate_copy(n_samples) if not self.reverse else self.generate_reverse(n_samples)
